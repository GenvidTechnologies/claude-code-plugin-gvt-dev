...
- **To exercise unreleased source without releasing, run a headless session against the working tree: `claude -p --plugin-dir plugin`.** It loads `plugin/` as `gvt-dev@inline`, and its agents, skills and hooks take precedence over the installed copy. On 2026-09-25 (#557), a validator dispatched this way quoted a Key Rules bullet that existed only in the working tree. Run it from a throwaway git repo when the behaviour under test touches git state. Two recipes proved useful:

  ```bash
  # Observe what a hook receives: a scratch settings file whose PreToolUse hook appends stdin to a log
  claude -p --settings "$SP/settings.json" --allowedTools "Bash(git status*)" "Agent" "<dispatch gvt-dev:validator to run git status --short>"
  # End-to-end against unreleased hooks/agents, with the command explicitly permitted so only a hook can block it
  claude -p --plugin-dir /abs/path/to/plugin --allowedTools "Bash(git stash*)" "Agent" "<dispatch a gvt-dev agent to attempt the command>"
  ```

  This is a verification tool for a change under development, not a substitute for the release-first rule above. An orchestrator you *run* in this session still executes from the installed cache.
...
So **"read the Warnings section and confirm it's empty" catches the first two and structurally cannot catch the third** — an un-indexed doc renders under `### Info (optional)` and reads like an unmet *optional expectation*, indistinguishable at a glance from the dozen benign ones this repo always carries. For step 7's `docs/TOC.md` row, **read `docs/TOC.md` and confirm the row is there** — the same rule the Testing section states for ADRs, and for the same reason: the audit is not a reliable oracle for index rows. Use the Warnings section for the README row and the description cap. Those two **warning**-severity findings are the author-time ones of the three, and they **auto-activate when the audit runs against the plugin repo itself** — the gate (`AUDITING_PLUGIN_SOURCE` in `audit-main.mjs`) is *path-derived* from the audited path being this repo's root, **not** an environment variable you export — so the same plain `node plugin/skills/audit-conventions/scripts/audit.mjs` from the repo root already includes them; there's nothing extra to enable, and a `Warnings` section is simply absent when there are none. **`orphaned-doc` is not author-time and is not gated:** `scanOrphanedDocs` is invoked alongside the other hygiene scanners, outside the gate block, so it fires in a *consumer's* audit too, against that consumer's own `docs/` tree. It appears in the table above for where it renders, not because it shares the other two's gating (see [ADR-0049](docs/decisions/0049-audit-core-mechanism-policy-boundary.md)). Note that "gated" does not mean "inside the block" — `desc-length` self-gates with an early return rather than sitting in it, which the paragraph below unpacks.
   ...
   **That covers an *illustrative* pointer; a *real* citation has its own rule, and it is the one that actually bites when authoring an ADR.** The paragraph above is about examples you write to demonstrate a defect. A genuine citation — a path and a line number you mean the reader to follow — must be **immediately followed by a content anchor**: a quoted span, a backticked identifier, or a colon-introduced quotation naming what that line should contain. The scanner then resolves the path and checks the anchor really is at that line, reporting *drift* (anchor found elsewhere, with the corrected number) or *broken* (anchor absent). **The cheapest way to comply is to cite with no line number at all** — a reference carrying an anchor and no number is not a pointer under the grammar, so it cannot decay and is never resolved:

   ```
   ADR-0049:61 ("Resolution is therefore a")   <- pointer: anchor checked
   ADR-0049, "Resolution is therefore a"        <- not a pointer: cannot decay
   ```

   **Do not infer the convention from the surrounding corpus — it will teach you the wrong thing.** Measured 2026-09-18: of 131 positional pointers repo-wide, **120 are bare** (no anchor) and live in 14 files, 9 of them ADRs. Every one of those 120 is *accepted debt* recorded in `.pointer-baseline.json`, not a pattern to copy; a new citation gets no such grace and fails `commands.validate` on the first run. So the dominant style in the tree is precisely the style you may not write. Reach for the anchored form, or drop the line number.
...
**`audit-main.mjs` itself is orchestration-only — put testable logic in `lib/`.** `audit.mjs` is just the entrypoint bootstrap (a dependency preflight, then a dynamic import of the body); `audit-main.mjs` is the body and has no exports, so its inline helpers (`evaluate*`, `formatReport`) aren't unit-testable. When a change adds logic worth pinning with a test, extract it into `lib/<name>.mjs` with a companion `test/<name>.test.mjs`, as the #151 summary-tally fix did (`lib/summary.mjs` + `test/summary.test.mjs`). One consequence to keep in mind: because `formatReport`'s exact rendered strings live in that un-exported body, the report *wording* has no test — only the extracted tally math does. Extracting the format itself (or an integration test that runs the script against a fixture) is the way to cover it.
