...
- **Verify a cross-reference's target before writing the pointer** — `development-principles.md` principle #12. A `see §X` / doc link / back-link asserts the target holds the thing; open it and confirm. If it doesn't, add the missing content rather than softening the wording. This bites hardest when the pointer *is* the fix for an ambiguity, since replacing a vague "handle it there" with a precise `→ §0c` upgrades a hedge into a falsifiable claim.
- **After adding or renaming headings, re-read the file's whole heading inventory** — `grep -n '^#\{1,4\} '` (or the format's equivalent) — and check **order** and **numbering namespace**, not just that the new heading exists. A presence grep passes for a heading that landed in the wrong place or whose number implies the wrong parent: a `### 3a` added under a `## 0` section both sorts ahead of that section's existing `### 0a`/`### 0b` siblings *and* reads as a child of `## 3`. Structured docs (skill bodies especially) are navigated by heading, so a mis-numbered one mis-routes the reader even though every content check is green. Match the local sibling scheme rather than inventing a parallel one.
  - **`^#` also matches inside fenced code blocks**, so the bare grep is unreliable on any doc that documents a markdown format — which is every bundled template describing a page or file structure. Exclude paired ` ``` `-delimited spans before reading the inventory (or, on a small file, just read it directly rather than grepping). The tell that an entry is fence content rather than a real heading: it's absent from the doc's own table of contents or section structure, or it appears only alongside other example-shaped siblings — an **unexpected second `# H1`** is the loudest instance, since a well-formed doc has exactly one.
- **A claim marked inherited or unverified in a dispatch is not settled fact — don't transcribe it into durable prose as though it were verified this run.** Before it lands in a doc, an ADR, or a CHANGELOG entry, either verify it yourself against the artifact it describes, or carry the hedge forward explicitly (attribute it to its source, or flag it as unconfirmed) rather than silently upgrading it to current state.
