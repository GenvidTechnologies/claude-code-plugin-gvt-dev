...
3. **Before removing or renaming a shared symbol, grep for ALL its call sites — the ones you were given are a sample** (`development-principles.md` #11). The `file:line` sites named in the task, the plan, or the issue read as exhaustive but are not; run an anchored, tree-wide grep yourself before editing. Anchor the pattern to how the symbol is *used* — a call `aceKey(`, a property `.aceKey`, a type `type aceKey` — rather than a bare `aceKey` that conflates `aceKeySet`/`aceByKey`. Don't lean on typecheck to catch the rest: a *deleted* definition does fail at the missed sites, but a value-preserving rename or a same-signature shadow slips through silently (construct3-chef #136: 3 sites listed, 9 actual). This matters most when no planner ran — a compressed pipeline, an orchestrator implementing inline, or a human-written plan carries no such directive.
4. **Implement** following existing patterns. Stay focused on the task; don't refactor outside its scope unless the plan calls for it.
5. **Write tests** if the task specifies them.
6. **Authored prose claims are output, not report — verify them against the source data before committing.** When the task or dispatch supplies structured facts — a table, measured figures, an enumerated case list — and asks you to render them as prose (a docstring, a code comment, an ADR, a commit body), reproduce or quote the values rather than paraphrasing them into flowing prose; the paraphrase is where a boolean gets inverted or a qualifier dropped. Nothing downstream catches it: no test, linter, or typechecker reads comment prose, so a false claim ships with every gate green. Rationale comments are the sharpest case — an inverted reason argues *for* the change the comment exists to prevent. And when you change behaviour that an earlier task in the same plan documented, re-read that prose — a comment accurate when written can be false by the time your commit lands, and the suite will not tell you because the assertion it sits above is unaffected either way.
7. **Diff hygiene before committing — don't reflow lines you didn't change** (`development-principles.md` #10). Match the surrounding code's existing style by hand; do not run a project-wide formatter (`prettier --write`, `eslint --fix` across files) just to tidy a file — it reflows unrelated pre-existing lines and pollutes the diff. Only run a formatter when it's the repo's **declared, installed gate**: the tool is in `devDependencies` AND a script runs it (a `format`/`lint:fix` in `package.json`, or a documented pre-commit step). When it is the gate, detect the package manager from lockfiles (`package-lock.json` → npm, `pnpm-lock.yaml` → pnpm, `yarn.lock` → yarn) and run it **scoped to the files you changed** (e.g. `<pm> exec prettier --write <modified-files>`). A bare config file (`.prettierrc*`) with no dependency and no script is **not** a gate — treat the linter as the only style authority and don't invoke the formatter at all.
