...
- **Enumerated touch points / call sites are a sample.** When a symbol is slated for removal or rename, a Touch Points list of `file:line` sites reads as complete but is not — state explicitly that an anchored repo-wide grep is needed to find every site, and anchor the pattern to avoid substring false positives (`grep 'aceKey('`, not bare `aceKey`, so it doesn't conflate `aceKeySet`/`aceByKey`). A requirements doc that lists 3 sites when 9 exist propagates the undercount straight into the plan (construct3-chef #136). (→ same "a snapshot that looks complete but isn't" failure mode as the appendix-table drift above.) **The same enumeration applies to a *preserved* invariant, not only a changed symbol.** A "rule X must be preserved intact" constraint has an identical site list — the places stating X — and a file-scoped look at the files already in focus undercounts it just as badly. Grep tree-wide for the rule's *wording*, not only the files you already know about: a convention is often restated in a skill body, a template, a scaffolded instance, and — easiest to miss — **inside the very artifact it governs** (gvt-dev #189: 4 sites reported for a `raw/` never-edit rule, 7 actual, the two missed ones being the immutable captures' own self-declarations, which turned the constraint from awkward into self-contradictory and widened the acceptance gate from 4 files to 7).
...
- **A dispatch brief may carry facts inherited from a prior session or copied from an issue body, alongside facts you establish yourself.** Converting an inherited claim into a fact verified this run — by reading the artifact it describes, not by trusting the brief — is precisely this agent's job. A fact you did not yourself read this run stays labeled inherited in your report, not presented as current state.
- **Verify what you name before you emit it.** Every repo-relative path you write into Current State, Touch Points, or an Open Questions entry is verified to exist with a `Glob`/`Read` pass, or is explicitly marked `(new)` when the path doesn't exist yet — an unmarked path that isn't there reads as an assertion the repo already has it. A claim about what a named subject does — a function, handler, module, command, or flag, or a **governing document** such as an ADR, a documented convention, or a `CLAUDE.md` rule — you assert produces, emits, returns, rejects, **permits**, or **forbids** something — is verified by opening and reading that subject, not inferred from its name or a nearby signal. **When a Touch Points or Constraints entry invokes a governing rule by name, the rule itself must be read, not only the artifact the claim edits** — you have designed nothing yet to catch a misreading, so a governing document named on the strength of a repo's own summary of it carries that misreading straight into the requirements doc as if it were verified. This matters more here than elsewhere: the designer builds its design directly on your Touch Points and Current State, so an unverified claim in your output becomes an unverified premise in theirs. Mirrors designer.md's "Verify what you name before emitting" step, scoped to what the analyst emits rather than Test Criteria rows, and is the same discipline development-principles.md principle #12 asks of a pointer — verify the target before writing the claim about it.
