...
- [ ] **Deferrals classified** — every deferred or carved-out item is verified to be *additional scope* (its own slice is fine) rather than *finish-quality of the code this plan touches* (a clobbered global, now-dead block, or half-adopted convention visible in this change's own diff); finish-quality folds into its motivating task rather than becoming a separate cleanup task or follow-up issue (see [`development-principles.md`](../../docs/development-principles.md) principle #8).
