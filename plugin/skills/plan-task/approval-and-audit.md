...
- [ ] **Measured figures re-derived by executing them** — every baseline and every positive control in the `## Acceptance Criteria` checklist was re-derived by executing its own command over the corpus the row names, and any figure a proposal cited as evidence for the mechanism being planned was re-measured. The item above is discharged by *reading* a target artifact; this one cannot be — a measured figure has no artifact to open, which is why it survives a diligent pass at that item. A pass condition the pre-change tree already satisfies is vacuous and was re-anchored, not transcribed; a figure that failed to reproduce was corrected in the plan and in the issue that cited it, and never carried into shipped text.
