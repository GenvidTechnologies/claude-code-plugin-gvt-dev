...
4. **Write the pre-committed acceptance criteria** — after the branch exists, before the prep commit. This step wholly precedes Execution's first implementer dispatch. **Determine the target issue** via a fallback chain: the `#N` referenced in the task description → a `plan-next-issue` handoff → the requirements doc's issue reference → else this is an issue-less run.
   - **Issue present (primary, Home B):** fetch the current issue body via `bugTracker.readOne` (e.g. `gh issue view {id} --json body`), then **append or insert** a `## Acceptance Criteria` section sourced from the planner's emitted checklist — **preserving the rest of the body; never raw-overwriting it** — and write it back with the host-native edit command for `bugTracker.kind` (e.g. `gh issue edit {id} --body-file {tmpfile}`, following `triage-issues`' body-file pattern). **Best-effort per `development-principles.md` principle #9:** if the write is blocked (auto-mode write-classifier denial, or no user present for a permission prompt), don't drop it — surface the fully drafted `## Acceptance Criteria` checklist so the user can apply it (`! gh issue edit …`) and record it as an outstanding action.
   - **Issue-less (fallback):** write `docs/acceptance/<slug>.md` (slug = the plan's kebab-case branch name) containing the checklist, and self-index it into `docs/TOC.md` under a conventional heading — the same scaffold-on-first-use pattern `docs/decisions/` follows below (offer interactively, auto in `--non-interactive`, idempotent, skip gracefully if `docs/TOC.md` is absent).
...
6. **Author a decision record when the ADR threshold is met.** Durable architecture and compromise rationale must survive the transient `plan.md` — capture it in a committed decision record (see `${CLAUDE_PLUGIN_ROOT}/docs/development-principles.md` principle #7).
   - **ADR threshold — author one *only* when both hold:** (a) Phase 2 (Design) actually ran, **or** the simple-task/full-proposal shortcut's `AskUserQuestion` checkpoint resolved a design fork that itself constitutes a non-trivial decision (a cross-module/mechanism choice, *or* a weighed-and-rejected alternative); **and** (b) a non-trivial decision was produced — a rejected alternative was weighed, *or* a cross-module/architectural choice was made. It's not *whether the formal designer ran* that gates the ADR, but *whether a non-trivial design decision was made* — regardless of which phase or checkpoint surfaced it. **Skip** the record otherwise (trivial, purely additive, or cosmetic changes — including shortcut tasks that resolved no genuine design fork — need none, which is what keeps the convention low-friction).
...
3. **Run `gvt-dev:validator`** on the staged changes after each implementation task — it now also checks the staged diff against the recorded `## Acceptance Criteria`.
...
**Independent tasks may run concurrently — but then commit with explicit pathspecs.** Tasks with no shared files and no ordering dependency can be dispatched in parallel (see `superpowers:dispatching-parallel-agents`) to cut wall-clock; tasks that share a file, or where one's output feeds another, stay sequential. When you *do* parallelize, the staged-but-uncommitted protocol has a sharp edge: every in-flight implementer stages into the **same git index**, so a bare `git commit -a` / `git add -A` at the end of one task **captures the sibling tasks' staged files too**, landing them in the wrong commit. Commit each task with **explicit pathspecs** (`git commit <that task's files>`) so the commit contains only its own work. And note the per-task validator gate under parallelism sees the **union** of all in-flight staged changes — it can't isolate one task — so keep parallel tasks genuinely independent and lean on the **final full-tree validate + end-of-run `code-reviewer`** as the real safety net rather than the per-task gate.
