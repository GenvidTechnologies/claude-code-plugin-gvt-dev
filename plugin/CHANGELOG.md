...
- **`code-reviewer`: the "run it or downgrade" rule now covers a finding's concrete *example*, not just its claim.** Rule 4 already required a deterministic-transform claim (regex match, slug, escaping, arithmetic) to be executed or filed with the uncertainty explicit — but the downgrade path let a review supply a specific unverified input at Warning severity, and nothing forced the hedge to actually appear. A wrong example is worse than none: it reads as the finding's concrete evidence, sends the orchestrator to fix a non-problem, and discredits the real observation. Reviews must now run an asserted input or describe the risk's shape in prose instead of inventing an illustrative case. Surfaced by a review of the `principle-citation` scanner that flagged `sub-principles #7` as a false positive — it isn't, since `-` is already a word boundary, and that is load-bearing because the principles doc's own name is hyphenated. Behavioral agent guidance → version bump at release.
